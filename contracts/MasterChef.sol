...
// MasterChef is the master of ThunderSwap Token (TNDR). He can make TNDR and he is a fair guy.
//
...
// contract once $TNDR is sufficiently distributed and the community can show to govern itself.
//
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 accThunderPerShare; // Accumulated TNDRs per share, times 1e12. See below.
        uint16 depositFeeBP; // Deposit fee in basis points
    }

    // The ThunderSwap Token!
    ...
    constructor(
        ...
        thunderPerBlock = INITIAL_EMISSION_RATE;
    ...
    function pendingThunder(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 thunderReward =
                multiplier.mul(thunderPerBlock).mul(pool.allocPoint).div(
                    totalAllocPoint
                );
            accThunderPerShare = accThunderPerShare.add(
                thunderReward.mul(1e12).div(lpSupply)
            );
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        thunder.mint(devAddr, thunderReward.div(10));
        thunder.mint(address(this), thunderReward);
        pool.accThunderPerShare = pool.accThunderPerShare.add(
            thunderReward.mul(1e12).div(lpSupply)
        );
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for TNDR allocation.
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if (user.amount > 0) {
            uint256 pending =
                user.amount.mul(pool.accThunderPerShare).div(1e12).sub(
                    user.rewardDebt
                );
            if (pending > 0) {
    ...
    // Safe thunder transfer function, just in case if rounding error causes pool to not have enough TNDRs.
    function safeThunderTransfer(address _to, uint256 _amount) internal {
        uint256 thunderBal = thunder.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > thunderBal) {
            transferSuccess = thunder.transfer(_to, thunderBal);
        } else {
            transferSuccess = thunder.transfer(_to, _amount);
        }
        require(transferSuccess, "safeThunderTransfer: transfer failed.");
    }
    ...
    function updateEmissionRate() public {
        ...
        require(thunderPerBlock > MINIMUM_EMISSION_RATE, "updateEmissionRate: Emission rate has reached the minimum threshold");
        ...
        uint256 newEmissionRate = thunderPerBlock;
        ...
        if (newEmissionRate >= thunderPerBlock) {
            return;
        }

        massUpdatePools();
        lastReductionPeriodIndex = currentIndex;
        uint256 previousEmissionRate = thunderPerBlock;
        thunderPerBlock = newEmissionRate;
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        ...
        if (referrer != address(0) && referrer != _user && refBonusBP > 0) {
            ...
            thunder.mint(referrer, refBonusEarned);
